...
public class ConcreteSyntax {
	...
	private Assignment assignment() {
		...
		if (token.getType().equals("Identifier")) {
			...
			token = input.nextToken(); // should be ":="
			match(":=");
			a.source = expression();
			match(";");
	...
	private Conditional ifStatement() {
		...
		if (token.getValue().equals("if")) {
			token = input.nextToken(); // consume 'if'
			match("(");                // match '('
			c.test = expression();     // parse the expression inside ()
			match(")");                // match ')'
			c.thenbranch = statement();      // parse the statement (body of if)
	...
	private Loop whileStatement() {
		...
		if (token.getValue().equals("while")) {
			token = input.nextToken(); // consume 'while'
			match("(");                // match '('
			l.test = expression();     // parse the expression inside ()
			match(")");                // match ')'
			l.body = statement();      // parse the statement (body of while)
		} else {
